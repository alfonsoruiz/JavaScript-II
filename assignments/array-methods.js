...
const largeShirts = runners.filter((obj) => {
...
let ticketPriceTotal = runners.reduce((accumulator, obj) => {
   return accumulator += obj.donation;
}, 0);
...
// Problem 1
// Get a count of how many runners are attending the race
let count = 0;
const totalRunners = runners.forEach((obj) => {
    count++;
});

console.log(count);
// Problem 2
// return a list of runners that gave donations of a hundred or more
const donationsOverHundred = runners.filter((obj) => {
    return obj.donation >= 100;
}, 0);

console.log(donationsOverHundred);

// Problem 3
// return runner with largest donation
let largestDonation = 0;
const largestDoner = runners.forEach((obj) => {
    if(obj.donation > largestDonation) {
        largestDonation = obj.donation;
    }
});
